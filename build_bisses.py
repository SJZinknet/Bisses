#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# BUILD_VERSION = "bisses-overview-segments-2026-06-06-b"
...
<body>
  ...
  <header class="topbar">
    <div class="brand">
      ...
      <div class="build-version">bisses-overview-segments-2026-06-06-b</div>
    </div>
    <div class="toolbar">
  ...
  <aside id="bisse-panel" class="panel panel-main">
    ...
    <div id="panel-content"><p class="muted">Cliquez sur une pastille pour afficher un bisse.</p></div>
  </aside>
  ...
  <aside id="context-panel" class="panel panel-context">
    ...
    <div id="context-content"><p class="muted">Cliquez sur une trace ou une photo.</p></div>
  </aside>
...
STYLES_CSS = r"""html,body{width:100%;height:100%;margin:0;padding:0}
body{overflow:hidden;font-family:Candara,"Segoe UI",system-ui,sans-serif;color:#1f2d24;background:#dfe5da}
#map{position:fixed;inset:0;z-index:1;width:100vw;height:100vh;height:100dvh;background:#dfe5da}
.leaflet-tile-pane{filter:brightness(1.10) saturate(.86) contrast(.93)}
button{font:inherit}
.topbar{position:fixed;z-index:700;top:16px;left:16px;right:16px;display:flex;justify-content:space-between;align-items:flex-start;gap:16px;pointer-events:none}
.brand,.toolbar,.panel,.legend,.status-pill,.scale-pill{pointer-events:auto}
.brand{max-width:min(430px,calc(100vw - 32px));padding:14px 18px;border-radius:22px;color:#fff;background:radial-gradient(circle at 15% 20%,rgba(255,255,255,.16),transparent 34%),linear-gradient(135deg,rgba(31,47,37,.96),rgba(77,99,70,.93));box-shadow:0 16px 42px rgba(20,30,22,.22);backdrop-filter:blur(8px)}
.eyebrow,.panel-kicker{margin:0 0 5px;text-transform:uppercase;letter-spacing:.14em;font-size:.72rem;font-weight:700;opacity:.72}
.build-version{margin-top:6px;font-size:.72rem;opacity:.55}
h1,h2,h3{margin:0;line-height:1.08}
h1{font-size:clamp(1.7rem,3vw,2.55rem)}h2{font-size:1.45rem}h3{font-size:1.13rem}
.toolbar{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}
.toolbar button,.close-button{border:1px solid rgba(66,58,43,.18);background:rgba(255,250,241,.96);color:#1f2d24;box-shadow:0 10px 28px rgba(20,30,22,.14);cursor:pointer}
.toolbar button{min-height:40px;padding:9px 14px;border-radius:999px}.toolbar button:hover,.close-button:hover{background:#fff}
.panel{position:fixed;z-index:650;border:1px solid rgba(66,58,43,.18);border-radius:24px;background:rgba(255,250,241,.96);box-shadow:0 18px 52px rgba(20,30,22,.22);backdrop-filter:blur(10px);overflow:auto}
.panel-main{left:16px;bottom:16px;width:min(390px,calc(100vw - 32px));max-height:calc(100dvh - 155px);padding:20px;transform:translateX(0);transition:transform .22s ease,opacity .22s ease}
.panel-main:not(.is-open){transform:translateX(calc(-100% - 28px));opacity:.1;pointer-events:none}
.panel-context{right:16px;bottom:16px;width:min(340px,calc(100vw - 32px));max-height:min(56dvh,520px);padding:18px;transform:translateY(0);transition:transform .18s ease,opacity .18s ease}
.panel-context:not(.is-open){transform:translateY(18px);opacity:0;pointer-events:none}
.panel-list{right:16px;top:82px;width:min(360px,calc(100vw - 32px));max-height:calc(100dvh - 106px);padding:18px;transform:translateX(0);transition:transform .2s ease,opacity .2s ease}
.panel-list:not(.is-open){transform:translateX(calc(100% + 28px));opacity:0;pointer-events:none}
.close-button{position:absolute;top:10px;right:10px;width:32px;height:32px;border-radius:999px;font-size:1.25rem;line-height:1}
.muted{color:#657064}.lead{margin:12px 0 14px;line-height:1.5}
.fact-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 12px;margin-top:14px}.fact{padding-top:9px;border-top:1px solid rgba(66,58,43,.17)}
.fact dt{color:#657064;font-size:.72rem;text-transform:uppercase;letter-spacing:.08em}.fact dd{margin:3px 0 0;font-weight:700}
.tags{display:flex;flex-wrap:wrap;gap:7px;margin-top:14px}.tag{display:inline-flex;border:1px solid rgba(66,58,43,.17);border-radius:999px;padding:5px 9px;background:#fff;font-size:.86rem}
.gallery{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:14px}.photo-card{border:1px solid rgba(66,58,43,.17);border-radius:15px;overflow:hidden;background:#fff;cursor:pointer;text-align:left;padding:0}
.photo-card img{width:100%;aspect-ratio:4/3;object-fit:cover;display:block}.photo-card div{padding:8px}.photo-card strong{display:block;font-size:.9rem}.photo-card span{display:block;margin-top:3px;color:#657064;font-size:.78rem;line-height:1.25}
#bisse-list{display:grid;gap:9px;margin-top:12px}.bisse-button{width:100%;border:1px solid rgba(66,58,43,.17);border-radius:16px;background:#fff;color:#1f2d24;padding:12px;text-align:left;cursor:pointer}.bisse-button:hover{border-color:rgba(63,98,69,.55)}.bisse-button strong{display:block}.bisse-button span{display:block;margin-top:3px;color:#657064;font-size:.86rem}
.legend{position:fixed;z-index:620;left:50%;bottom:18px;transform:translateX(-50%);display:flex;flex-wrap:wrap;justify-content:center;gap:7px;max-width:min(760px,calc(100vw - 40px));padding:8px;border:1px solid rgba(66,58,43,.18);border-radius:999px;background:rgba(255,250,241,.94);box-shadow:0 12px 34px rgba(20,30,22,.18);backdrop-filter:blur(8px)}
.legend:empty{display:none}.legend-item{display:inline-flex;align-items:center;gap:6px;min-height:27px;padding:4px 9px;border:1px solid rgba(66,58,43,.17);border-radius:999px;background:#fff;font-size:.86rem}.legend-swatch{width:12px;height:12px;border-radius:50%}
.status-pill,.scale-pill{position:fixed;z-index:620;max-width:min(420px,calc(100vw - 32px));padding:8px 11px;border-radius:999px;color:#fff;font-size:.84rem;box-shadow:0 10px 30px rgba(20,30,22,.22)}
.status-pill{left:16px;bottom:16px;background:rgba(31,45,36,.88)}.scale-pill{right:16px;bottom:16px;background:rgba(31,45,36,.78)}.status-pill.is-hidden,.scale-pill.is-hidden{display:none}
.bisse-marker{width:30px;height:30px;border:3px solid #fff;border-radius:50%;background:#365f40;box-shadow:0 4px 15px rgba(0,0,0,.35)}
.photo-marker{width:18px;height:18px;border:2px solid #fff;border-radius:50%;background:radial-gradient(circle at center,#fff 0 2px,transparent 3px),#1f2d24;box-shadow:0 4px 12px rgba(0,0,0,.3)}
.leaflet-tooltip.bisse-tooltip,.leaflet-tooltip.segment-tooltip{border:0;border-radius:999px;padding:6px 9px;background:rgba(31,45,36,.94);color:#fff;box-shadow:0 8px 22px rgba(0,0,0,.20);font-family:Candara,"Segoe UI",system-ui,sans-serif;font-size:.88rem}
.context-row{padding:9px 0;border-top:1px solid rgba(66,58,43,.17)}.context-row strong{display:block;color:#657064;font-size:.72rem;text-transform:uppercase;letter-spacing:.08em}.context-row span{display:block;margin-top:3px}.context-photo{width:100%;border-radius:16px;margin-bottom:12px;display:block}
.error-box{padding:12px;border:1px solid rgba(150,62,48,.35);border-radius:16px;color:#76382f;background:rgba(150,62,48,.08)}
@media(max-width:760px){.topbar{display:block}.brand{width:fit-content;margin-bottom:8px}.toolbar{justify-content:flex-start}.panel-main{width:calc(100vw - 32px);max-height:48dvh}.panel-context{left:16px;right:16px;width:auto}.legend{display:none}.fact-grid,.gallery{grid-template-columns:1fr}}
"""
...
def main() -> None:
    ...
    print("Version : bisses-overview-segments-2026-06-06-b")
